...
#include "timer.h"
#include "mm_hydraulics_control.h"

// Mesh Mould Hydraulics objects
MMHydraulicsControl mm_hydraulics_control;
...
const int MSG_CMD_MOTOR_ON_OFF = 9; //[int1, int2] # int1 = motor state, int2 = motor num
// ====================================================================
...
void setup() {
  ...
  // Setup up Mesh Mould Hydraulics Control
  mm_hydraulics_control.Setup();

  // Start interrupt timer for control loop
  StartTimer(TC1, 0, TC3_IRQn, mm_hydraulics_control.frequency());
  
}
// ==================================================================

// Control loop interrupt timer callback function
void TC3_Handler()
{
  // Calling TC_GetStatus to "accept" interrupt
  TC_GetStatus(TC1, 0);
  
  // Spin one ciycle of control loop
  mm_hydraulics_control.SpinOnce();
}
...
void loop() {
  ...
  if (client) {
    ...
    while (client.connected()) { //while (CONNECTED) {
      ...
      // Sending control variables to serial
      ControlVariables control_variables_l, control_variables_r;
      int state;
      
      noInterrupts();
      control_variables_l = mm_hydraulics_control.control_variables_left();
      control_variables_r = mm_hydraulics_control.control_variables_right();
      state = mm_hydraulics_control.state();
      interrupts();
    
      //Serial.print(state); Serial.print(" ");
      Serial.println(control_variables_l.x_ref); //Serial.print(" ");
      //Serial.println(control_variables_l.u);
