...
void interrupt_setup() {
  ...
  // stop pin
  attachInterrupt(digitalPinToInterrupt(pin_sig_stop), set_stop_state, CHANGE);
  // weld started pin
...
bool wait_for_hydraulic_status(byte status_des, int timeout_ms) // wait for the desired hydraulics status with a timeout
{
  ...
  { 
    // test the interrupt
    test_pause_state_h(status_des, &time_start);
...
// interrupt functions ====================================================
void test_pause_state_h(byte des_state, unsigned long *time_start) {
  if (state_enable == LOW) {
...
void test_pause_state(boolean pausing) {
