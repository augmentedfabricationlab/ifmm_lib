...
const int MSG_CMD_BEND = 14; // # [int1, int2]  # int1 = abs motor value for rot front, # int2 = abs motor value for rot back --> for now: rotates both  motors at the same time
const int MSG_CMD_RESET_ENCODER = 15; // [] empty message to reset the encoder
const int MSG_CMD_GET_ENCODER_VAL = 16; // [double] returns the relative angle in nr of ticks ==> conversation has to be done on server side

// ====================================================================

// conversions ========================================================
long bytesToLong(byte b[4]) {
  long val = 0;
  val = ((long )b[3]) << 24;
  val |= ((long )b[2]) << 16;
  val |= ((long )b[1]) << 8;
...
//
void send_long(EthernetClient c, long l) {
  byte bbuffer[4];
  longToBytes(l, bbuffer);
  for (int i = 0; i < 4; i++) {
    c.write(bbuffer[i]);
  }
}
...
void send_msg_header(EthernetClient c, long msg_len, long msg_id, long msg_counter) {
  send_long(c, msg_len); // 1. send msg length // = 4 bytes
  send_long(c, msg_id);  // 2. send msg type // = 4 bytes
  send_long(c, msg_counter); // 3. send msg counter // = 4 bytes
}
//
...
void send_rel_enc_pos(EthernetClient c, long msg_counter, long encoder_val) {
  Serial.println("SENDING MSG_CMD_GET_ENCODER_VAL");
  long msg_length = 8; //4 bytes for msg_counter and 4 bytes for encoder val
  send_msg_header(c, msg_length,MSG_CMD_GET_ENCODER_VAL, msg_counter);
  send_long(c,encoder_val);
}
...
void setup() {
  ...
  // set up the motor
  motors_setup();
  // set up ERN1020 encoders
  encoder_setup();
  //set up the interrupts/pins from the robot control relais
  interrupt_setup();
      ...
      if (client.available() > 0) { //if bytes are available on the socket
        ...
        switch (msg_id) {
          ...
          case MSG_CMD_WELD:
            ...
            if (msg_wait_for_response == 1) {
              if (cb_success == true) {
                ...
                Serial.println("SEND EXEC WITH -1");
                send_msg_cmd_exec(client, -1); //the -1 is read by the python client, and detects a mistake
              }
            }
 
            break;
            
          case MSG_CMD_GET_ENCODER_VAL:
            Serial.println("MSG TYPE: MSG_CMD_GET_ENCODER_VAL");
            //send back msg received
            send_msg_cmd_received(client, msg_counter);
            //exec callback function
            long encoder_val_copy;
            cb_get_encoder_val(&encoder_val_copy);
            delay(1); //is it necessary?
            //Serial.println(encoder_val_copy);
            send_rel_enc_pos(client,msg_counter,encoder_val_copy);
            break;          
          ...
          case MSG_CMD_FEED_WIRE:
            ...
            send_msg_cmd_received(client, msg_counter);
            //exec callback function
            cb_success = cb_feed_wire(int_value);
