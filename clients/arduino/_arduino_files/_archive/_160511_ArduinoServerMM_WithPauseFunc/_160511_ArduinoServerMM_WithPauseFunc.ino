...
//const int relay_led = 13; //9 for arduino ethernet
const int di_interrupt = 13; // for reading the state of the pause switch
int di_interrupt_state = 0;

// server setup =========================================================
byte mac[] = {0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED };
IPAddress ip(192, 168, 125, 177);
...
const int MSG_CMD_BEND = 3;
const int MSG_CMD_INSERT_VERTICAL = 4;
const int MSG_CMD_RETRACT = 5;
const int MSG_DIGITAL_OUT = 6;
const int MSG_FLOAT_LIST = 7;
const int MSG_STOP = 8;
const int MSG_INFO = 9;
const int MSG_STRING = 10;
const int MSG_CMD_INSERT_AND_BEND = 11;  //[int1, int2] # int1 = long or short vertical boolean, int2 = absolute motor value for bending
const int MSG_CMD_ROTATE_MOTOR_ABS = 12; //[int1, int2] # int1 = absolute motor value for rotation, int2 = motor front or motor back

// New
const int MSG_CMD_ROTATE_MOTOR_REL = 13; //[int1, int2] # int1 = relative motor value for rotation, int2 = motor front or motor back
const int MSG_CMD_INSERT_VERTICAL_BEF_TILT = 14; //[float] # = long or short vertical
const int MSG_CMD_BEND_AFTER_TILT = 15; //[float] # = bending angle
...
//
void send_msg_header(EthernetClient c, long msg_len, long msg_type, long msg_counter) {
  send_int(c, msg_len); // 1. send msg length // = 4 bytes
  send_int(c, msg_type);  // 2. send msg type // = 4 bytes
...
//
void read_msg_cmd_iab_from_socket(EthernetClient c, long int_vals[2]) {
  // 1. read int vals // = 2 * 4 bytes
  for (int i = 0; i < 2; i++) {
    int_vals[i] = read_int(c);
  }
}
      ...
      if (client.available() > 0) { //if bytes are available on the socket
        ...
        switch (msg_type) {
          ...
          case MSG_CMD_BEND:
            ...
            float fval_bend[1];
            read_msg_cmd_from_socket(client, fval_bend);
            //send back msg received
            send_msg_cmd_received(client, msg_counter);
            //exec callback function
            cb_exec_cmd_bend(fval_bend);
            //send back msg cmd executed
            if (msg_wait_for_response == 1) {
              send_msg_cmd_exec(client, msg_counter);
            }
            break;
          case MSG_CMD_INSERT_VERTICAL:
            Serial.println("MSG TYPE: MSG_CMD_INSERT_VERTICAL");
            float fval_weld[1];
            read_msg_cmd_from_socket(client, fval_weld);
            //send back msg received
            send_msg_cmd_received(client, msg_counter);
            //exec callback function
            cb_exec_cmd_insert_vertical(fval_weld);
            //send back msg cmd executed
            if (msg_wait_for_response == 1) {
              send_msg_cmd_exec(client, msg_counter);
            }
            break;
          case MSG_CMD_RETRACT:
            Serial.println("MSG TYPE: MSG_CMD_RETRACT");
            float fval_retract[1];
            read_msg_cmd_from_socket(client, fval_retract);
            //send back msg received
            send_msg_cmd_received(client, msg_counter);
            //exec callback function
            cb_exec_cmd_retract(fval_retract);
            //send back msg cmd executed
            if (msg_wait_for_response == 1) {
              send_msg_cmd_exec(client, msg_counter);
            }
          ...
          case MSG_DIGITAL_OUT:
            ...
            //read_msg_do_from_socket(client);
            long do_state[2];
            ...
            break;
          case MSG_CMD_INSERT_AND_BEND:
            Serial.println("MSG TYPE: MSG_CMD_INSERT_AND_BEND");
            //read_msg_do_from_socket(client);
            long int_vals[2];
            read_msg_cmd_iab_from_socket(client, int_vals);
            //send back msg received
            send_msg_cmd_received(client, msg_counter);
            //exec callback function
            cb_insert_and_bend(int_vals);
            //send back msg cmd executed
            if (msg_wait_for_response == 1) {
              send_msg_cmd_exec(client, msg_counter);
            }
          ...
          case MSG_CMD_INSERT_VERTICAL_BEF_TILT:
            Serial.println("MSG TYPE: MSG_CMD_INSERT_VERTICAL_BEF_TILT");
            float fval_insert_vert_bef_tilt[1];
            read_msg_cmd_from_socket(client, fval_insert_vert_bef_tilt);
            //send back msg received
            send_msg_cmd_received(client, msg_counter);
            //exec callback function
            cb_exec_cmd_insert_vert_bef_tilt(fval_insert_vert_bef_tilt);
            //send back msg cmd executed
            if (msg_wait_for_response == 1) {
              send_msg_cmd_exec(client, msg_counter);
            }
          ...
          case MSG_CMD_BEND_AFTER_TILT:
            Serial.println("MSG TYPE: MSG_CMD_BEND_AFTER_TILT");
            float fval_bend_after_tilt[1];
            read_msg_cmd_from_socket(client, fval_bend_after_tilt);
            //send back msg received
            send_msg_cmd_received(client, msg_counter);
            //exec callback function
            cb_exec_cmd_fval_bend_after_tilt(fval_bend_after_tilt);
            //send back msg cmd executed
            if (msg_wait_for_response == 1) {
              send_msg_cmd_exec(client, msg_counter);
            }
