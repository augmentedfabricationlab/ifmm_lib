...
int m_feeder_speed = 1500; //steps per second
int m_feeder_accel = 600; //steps per second
int m_feeder_max_speed = 1550; //steps per second
...
void interrupt_setup() {
  ...
  // weld exec pin
  attachInterrupt(digitalPinToInterrupt(pin_sig_weld_exec), set_weld_exec_state, RISING);
}
...
bool weld() {
  ...
  if (ok == true)
  {
    ...
    // now wait until weld_state_exec is set to high
    while (state_weld_exec == false){
          Serial.println("waiting for manual input (weld exec signal)");
          delay(200);
