...
class ClientRobot(ClientGeneric): #former: inherited from Thread
    ...
    def send(self, msg_type, msg = None):
        ...
        if msg_type == MSG_COMMAND:
            ...
            print "Message identifier unknown:  %s = %d, message: %s" % (arm_msg_types_str_array[msg_type], msg_type, msg)
            return   
           
        if buf != None:
            self.socket.send(buf)
            print "%s: Sent message: %s to server " % (self.parent.identifier, arm_msg_types_str_array[msg_type])
    ...
    def _send_command(self, cmd): 
        ...
        [msg_type, msg_counter, timestamp_sec, timestep_nanosec, cmd_type, float1-float7, int_vel, float_duration, int_zone, int_tool, float_arbitrary] 
        """
        print "cmd", cmd
        msg_snd_len = struct.calcsize(str(len(cmd)) + "f") + 12   
        params = [msg_snd_len, MSG_COMMAND] + self.get_header() + cmd
        print "params", params
        buf = struct.pack(self.byteorder + "2Q" + "3I" + "i" + "7f" + "if2if", *params)     
        self.socket.send(buf)
    ...
    def process(self, msg_len, msg_type, raw_msg):
        ...
        if msg_type == MSG_COMMAND_RECEIVED:
            ...
            msg = struct.unpack_from(self.byteorder + "7f", raw_msg)
        elif msg_type == MSG_CURRENT_POSE_JOINT:
            msg = struct.unpack_from(self.byteorder + "6f", raw_msg)
        elif msg_type == MSG_STRING:
            msg = raw_msg   
        elif msg_type == MSG_FLOAT_LIST: 
    ...
    def process_msg_cmd_executed(self, cmd_counter=None):
        ...
        if not len(self.parent.stack) and self.get_cmd_counter() == cmd_counter: #if not len(self.parent.stack) and self.get_stack_counter() == -self.stack_size and self.get_waypoint_counter() == msg_counter:
            print "Socket: self.publish_state(STATE_READY)"
    ...
    def handle_stack(self):
        if len(self.parent.stack) and self.get_stack_counter() == 0 :
            ...
            #print "Socket: The actuator needs to accomplish %i step%s in total." % (len(self.parent.stack), "s" if len(self.parent.stack) > 1 else "")
            ...
            #print "Socket: The actuator has still %d step%s to accomplish." % (len(self.parent.stack) + self.get_stack_counter() * -1, "s" if len(self.parent.stack) > 1 else "")
            pass
    #=================================================================================
