...
class MMFabMesh(MMMesh):
    ...
    def generate_nodes_in_layers(self, row_from, row_to):
        ...
        nodes_in_row_srf1 = [self.get_fab_nodes_row(row, index_row, "srf1", index_layer_list=0) for index_row, row in enumerate(range(max(row_from,1), row_to))]
        nodes_in_row_srf2 = [self.get_fab_nodes_row(row, index_row, "srf2", index_layer_list=1) for index_row, row in enumerate(range(max(row_from,1), row_to))]
        
        return zip(nodes_in_row_srf1, nodes_in_row_srf2)
    
    #===========================================================================
    def generate_nodes_in_layers_with_interiors(self, row_from, row_to):
        '''calculcate the nodes in layers, starting with srf1, srf2 and each x row interiors'''
    ...
    def generate_nodes_exterior_lead_out(self, row, index_row, node_type, srf, layer_type, index_layer_list):
        ...
        wrist_plane_for_bending_2 = rg.Plane(wrist_plane_for_bending_1)
        wrist_plane_for_bending_2.Translate(wrist_plane_for_bending_2.ZAxis * offset_lead_out_z)
        
        node_lead_out_2.fill_params(wrist_plane_for_bending_2)  
        
        return [node_lead_out_0, node_lead_out_1, node_lead_out_2]
    ...
    def get_corner_pts_interior(self, row, col, srf = "srf1"):
        ...
        pt1_row_current = pt1_row_current - vec_v/2*1
        pt2_row_current = pt2_row_current + vec_h/4*1 - vec_v/2*1
        pt2_row_under = pt2_row_under + vec_h/4*1
    ...
    def calc_bending_angles_row_2(self, nodes_in_row):
    ...
    def calc_bending_angles_row(self, nodes_in_row):
        ...
        for i in range(len(nodes_in_row)-1):
            ...
            if nodes_in_row[i+1].line_continuous_element:
                ...
                angle = rg.Vector3d.VectorAngle(node_current.wrist_plane_for_bending.XAxis, vec, node_current.wrist_plane_for_bending)
